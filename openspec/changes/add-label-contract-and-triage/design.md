...
- Keep the judgement swappable for Jev without touching contract or threshold; the report changes only in the tier it records (D4).
...
The judgement is `claude -p --safe-mode --tools "" --strict-mcp-config --no-session-persistence --model <m> --effort <e> --output-format json --json-schema <schema>` with a prompt built from `src/label-triage/criteria.md` (the contract's meanings, in the words the future creation-time skill will reuse) plus a batch of tasks. The prompt carries issue text anyone can write, so the session runs with every customisation off, no tools, no MCP servers and no transcript (`--bare` would also drop the keychain OAuth the subscription needs), and the prompt tells the model that the task fields (title, description, labels) are untrusted data to classify, never instructions to follow. The schema returns, per task id, per group: `labels`, `confidence`, `reason`. The envelope of `--output-format json` and the structured payload are both validated strictly; a task missing from the answer, or given a label outside the vocabulary, is reported as not judged. The exec is `JudgementExec = (batch) => Promise<Result>` and is what the Jev sub-issue replaces; that sub-issue must also change the tier recorded for judged tasks and for the step, which is set outside the exec and today hard-coded `llm` (`src/label-triage/judgement.ts`, `src/label-triage/step.ts`; the `Tier` of `rules.ts` has no `jev`), while contract, threshold and the rest of the report stay as they are. Timeout 300 s per batch. `claude -p` uses the active account and the subscription: the README states `--account` must be that account.
...
- [Issue titles and bodies are untrusted text in the judgement prompt] → safe mode, no tools, no MCP servers, no session persistence, and an instruction to classify, never obey.
